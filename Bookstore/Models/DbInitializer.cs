...
namespace Bookstore.Models
{
    ...
    {
        public static void Initialize(BookStoreDbContext context, String imageDirectory)
        {
            //Check if Database exists
            //és
            // Add every migration
            context.Database.Migrate();
            //If "Books" contains anything we don't initialize
            if(context.Books.Any())
            {
                return;
            }
            //Create a default list of books
            ...
            for (int i = 0; i < defaultBookList.Count(); i++)
            {
                String filepath = Path.Combine(imageDirectory, defaultBookList[i].Title) + ".png";
                defaultBookList[i].CoverImage = File.Exists(filepath) ? File.ReadAllBytes(filepath) : null;
            }
            context.Books.AddRange(defaultBookList);
            //Every books has 5 volumes
            for (int i = 0; i < defaultBookList.Count(); i++)
            {
                for (int j = 0; j < 5; j++)
                {
